...
const mongoActions_1 = require("./mongoActions");
...
router.post('/message', async (req, res) => {
    ...
    try {
        ...
        const aiResult = await (0, logic_1.extractIntent)(userMessage, conversationHistory);
        console.log('AI result:', aiResult);
        // 2. Perform DB action based on intent
        let dbResult = null;
        let aiReply = '';
        switch (aiResult.intent) {
            case 'create_ride':
                ...
                dbResult = await (0, mongoActions_1.createRide)(createData);
                break;
            case 'search_rides':
                dbResult = await (0, mongoActions_1.findRides)({
                    origin: aiResult.origin,
                    destination: aiResult.destination,
                    date: aiResult.date,
            ...
            case 'get_ride':
                ...
                dbResult = await (0, mongoActions_1.findRideById)(aiResult.rideId);
                if (!dbResult) {
                ...
                break;
            case "book_ride":
                const authReq = req; // cast Request to AuthRequest
                if (!authReq.user) {
                ...
                const ride = await (0, mongoActions_1.findRideById)(aiResult.rideId);
                if (!ride) {
                ...
                await (0, mongoActions_1.updateRide)(aiResult.rideId, { seats: seatsAvailable - seatsRequested });
                // create booking
                dbResult = await (0, mongoActions_1.createBooking)(null, aiResult.rideId, seatsRequested);
        ...
        aiReply = await (0, logic_1.generateUserReply)(dbResult, updatedConversation);
